## GOAL: run 100 pvl scans - one for each of my simulated traits files.
...
#phe1 <- expr.mrna[, which(colnames(expr.mrna) == "ENSMUSG00000038147")]
...
# note that we've uploaded 100 files, each of which contains a bivariate trait.
  fn <- paste0("2017-12-07_Ysim-run3_", proc_num, ".txt")

  PATH_TO_SIM_DATA <- "2017-12-07_run3-400sims"
...
fn <- paste0("2017-12-07-pvl400-run3_", proc_num, ".txt")
#save(list = "out", file = fn)
write.table(out, fn, quote = FALSE)
