...
phenames <- c("Cd84 expression", "Adarb2 expression")

load(file.path(PATH_TO_DATA, "expr.mrna.RData"))

phe1 <- expr.mrna[, which(colnames(expr.mrna) == "ENSMUSG00000038147")]

phe2 <- expr.mrna[, which(colnames(expr.mrna) == "ENSMUSG00000052551")]
...
fn <- paste0("2017-10-26_boot3_", proc_num, "_", paste(phenames, collapse = "_"), ".txt")
write.table(lrt, fn)
q("no")
